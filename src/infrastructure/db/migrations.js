...
export const runMigrations = async (db) => {
  ...
  `);
  const currentVersionRow = await db.getFirstAsync('SELECT value FROM metadata WHERE key = ?', ['db_version']);
  ...
  for (const migration of migrations) {
    if (migration.version > currentVersion) {
      ...
      await db.runAsync('INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)', ['db_version', String(currentVersion)]);
    }
  }
};
